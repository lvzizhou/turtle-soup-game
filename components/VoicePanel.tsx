...
export default function VoicePanel({ code, playerId, nickname }: { code: string; playerId: string; nickname?: string }) {
 ...
 const connect = async () => { if (reconnecting.current || ref.current?.state === 'connected') return; reconnecting.current = true; setError(''); try {
  ...
  room.on(RoomEvent.Disconnected, () => { reconnecting.current = false; ref.current = undefined; clearAudio(); activeTimers.current.forEach(timer => window.clearTimeout(timer)); activeTimers.current.clear(); setSpeakers([]); setJoined(false); window.setTimeout(() => { if (document.visibilityState === 'visible') connect(); }, 2000); }); await room.connect(d.url, d.token); reconnecting.current = false; setJoined(true); try { await room.localParticipant.setMicrophoneEnabled(true); setMicEnabled(true); } catch { setMicEnabled(false); setError('语音已连接，但麦克风暂不可用，请点击麦克风重试'); } speakerRef.current = true; setSpeakerEnabled(true);
 ...
 const toggleMic = async () => { if (!ref.current || !joined) return; const next = !micEnabled; try { await ref.current.localParticipant.setMicrophoneEnabled(next); setMicEnabled(next); setError(''); } catch { setError('麦克风暂不可用，请检查浏览器权限'); } };
