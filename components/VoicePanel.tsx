...
export default function VoicePanel({ code, playerId, nickname }: { code: string; playerId: string; nickname?: string }) {
 ...
 const connect = async () => { if (reconnecting.current || ref.current?.state === 'connected') return; reconnecting.current = true; setError(''); try {
  ...
  room.on(RoomEvent.Disconnected, () => { reconnecting.current = false; ref.current = undefined; clearAudio(); activeTimers.current.forEach(timer => window.clearTimeout(timer)); activeTimers.current.clear(); setSpeakers([]); setJoined(false); window.setTimeout(() => { if (document.visibilityState === 'visible') connect(); }, 2000); }); await room.connect(d.url, d.token); reconnecting.current = false; setJoined(true); try { await room.localParticipant.setMicrophoneEnabled(micRef.current); setMicEnabled(micRef.current); } catch { setError('麦克风暂不可用，请检查浏览器权限'); } speakerRef.current = speakerEnabled; setSpeakerEnabled(speakerRef.current);
 } catch (e: any) { reconnecting.current = false; ref.current = undefined; setError(e.message || '语音连接失败'); } };
 ...
 const toggleMic = async () => { const next = !micRef.current; micRef.current = next; setMicEnabled(next); if (!ref.current || !joined) return; try { await ref.current.localParticipant.setMicrophoneEnabled(next); setError(''); } catch { setError('麦克风暂不可用，请检查浏览器权限'); } };
 const toggleSpeaker = () => { const next = !speakerRef.current; speakerRef.current = next; elements.current.forEach(el => { el.muted = !next; el.volume = next ? 1 : 0; }); setSpeakerEnabled(next); };
 return <div className="voice-panel"><div className="voice-actions"><button onClick={toggleSpeaker} className={`voice-icon ${speakerEnabled ? '' : 'voice-icon-off'}`} aria-label={speakerEnabled ? '关闭听筒' : '打开听筒'} title={speakerEnabled ? '听筒：已开启' : '听筒：已关闭'}>{speakerEnabled ? '🔊' : '🔇'}</button><button onClick={toggleMic} className={`voice-icon ${micEnabled ? '' : 'voice-icon-off'}`} aria-label={micEnabled ? '关闭麦克风' : '打开麦克风'} title={micEnabled ? '麦克风：已开启' : '麦克风：已关闭'}>{micEnabled ? '🎙️' : '🔇'}</button></div><div className="voice-speaker-list" aria-live="polite">{speakers.map(speaker => <div className="voice-speaker" key={speaker.id}><span className="voice-speaking-dot" />{speaker.label}<span>正在说话</span></div>)}</div>{error && <small className="voice-error">{error}</small>}<div ref={audio} /></div>;
}
